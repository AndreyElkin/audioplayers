import React from 'react';
import type { BaseIconProps } from '../../../types/icon';

interface NextTrackIconProps extends BaseIconProps {
  /** Направление иконки: right (вперед) или left (назад) */
  direction?: 'right' | 'left';
  /** Состояние: активна/неактивна (например, если нет следующей песны) */
  disabled?: boolean;
}

const NextTrackIcon: React.FC<NextTrackIconProps> = ({
  size = 16,
  strokeColor = '#AAAAAA',
  strokeWidth = 1,
  fillColor = '#AAAAAA',
  direction = 'right',
  disabled = false,
  isActive = false,
  className = '',
  style,
  label = 'Следующая песня',
  variant = 'default',
  ...props
}) => {
  const activeColor = isActive ? '#FC6D3E' : fillColor;
  const iconColor = disabled ? '#CCCCCC' : activeColor;
  
  // Поворот для направления "left" (предыдущая песня)
  const rotation = direction === 'left' ? 180 : 0;

  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://.w3.org/2000/svg"
      className={`icon next-icon direction-${direction} ${disabled ? 'disabled' : ''} ${className}`.trim()}
      style={{
        transform: `rotate(${rotation}deg)`,
        opacity: disabled ? 0.5 : 1,
        cursor: disabled ? 'not-allowed' : 'pointer',
        ...style,
      }}
      aria-label={label}
      role="img"
      {...props}
    >
      <title>{direction === 'right' ? 'Следующая песня' : 'Предыдущая песня'}</title>
      
      <path
        d="M13 2.5V13.5C13 13.6326 12.9473 13.7598 12.8536 13.8536C12.7598 13.9473 12.6326 14 12.5 14C12.3674 14 12.2402 13.9473 12.1464 13.8536C12.0527 13.7598 12 13.6326 12 13.5V8.89151L4.52148 13.4618C4.36989 13.5544 4.19636 13.605 4.01873 13.6083C3.84111 13.6116 3.6658 13.5676 3.51085 13.4807C3.3559 13.3938 3.2269 13.2672 3.13711 13.1139C3.04733 12.9606 3 12.7861 3 12.6085V3.39154C3 3.21389 3.04732 3.03944 3.1371 2.88615C3.22688 2.73285 3.35588 2.60624 3.51082 2.51934C3.66576 2.43243 3.84106 2.38837 4.01868 2.39169C4.1963 2.39501 4.36983 2.44558 4.52142 2.53821L12 7.10846V2.5C12 2.36739 12.0527 2.24021 12.1464 2.14645C12.2402 2.05268 12.3674 2 12.5 2C12.6326 2 12.7598 2.05268 12.8536 2.14645C12.9473 2.24021 13 2.36739 13 2.5Z"
        fill={iconColor}
      />
    </svg>
  );
};

NextTrackIcon.displayName = 'NextIcon';

export default NextTrackIcon;